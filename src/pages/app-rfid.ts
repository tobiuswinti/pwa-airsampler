...
type IconState  = '' | 'success' | 'error';
type ScanStatus = 'idle' | 'scanning' | 'detected' | 'error' | 'unavailable';

// ── Component ──────────────────────────────────────────────────────────────
@customElement('app-rfid')
export class AppRfid extends LitElement {

  /* ── State ── */
  @state() private scanStatus:  ScanStatus = 'idle';
  @state() private statusText   = 'Ready to scan';
  @state() private iconState:   IconState  = '';
  @state() private tags:        TagRecord[] = [];
  @state() private browserInfo  = '—';
  @state() private nfcAvailable = true;
  @state() private showCompat   = false;
  @state() private compatMsg    = '';
  ...
  private _checkBrowser() {
    ...
    const chromeVer = isChrome ? parseInt(chromeMatch![1]) : 0;
    const isAndroid = /Android/.test(ua);

    this.browserInfo = isChrome
      ? `Chrome ${chromeVer} · ${isAndroid ? 'Android' : 'Desktop'}`
      : 'Unsupported Browser';

    if (!('NDEFReader' in window)) {
      this.nfcAvailable = false;
      this.showCompat   = true;
      this.scanStatus   = 'unavailable';
      this.statusText   = 'NFC not available';
      this.compatMsg    =
        '⚠ Browser not supported. Web NFC requires Chrome 89+ on Android. ' +
        'Desktop browsers and Safari/Firefox do not support the Web NFC API.';
    }
  }

  /* ── Scan controls ── */
  private async _startScan() {
    if (!this.nfcAvailable) return;

    this.abortController = new AbortController();
    this.scanStatus  = 'scanning';
    this.statusText  = 'Scanning — hold tag near device…';
    this.iconState   = '';
    ...
    try {
      ...
      reader.addEventListener('reading', ({ message, serialNumber }: any) => {
        this._onTagRead(message, serialNumber);
      });
      ...
      if (err.name === 'AbortError') {
        ...
        this.showCompat  = true;
        this.compatMsg   =
          '⚠ Permission denied. Allow NFC access when prompted, or check your browser settings.';
      } else {
  ...
  private _stopScan(resetStatus = true) {
  ...
  private _onTagRead(message: any, serialNumber: string) {
    this.statusText = 'Tag detected!';
    this.iconState  = 'success';
    this.scanStatus = 'detected';

    const decoded: DecodedRecord[] = message.records.map((r: any) => this._decodeRecord(r));
  ...
  /* ── NDEF decode ── */
  private _decodeRecord(record: any): DecodedRecord {
    const { recordType, mediaType, encoding, lang, data } = record;
    const out: DecodedRecord = { type: recordType, enc: null, lang: null, value: null, isHex: false };

    try {
      if (recordType === 'text') {
        out.value = new TextDecoder(encoding || 'utf-8').decode(data);
        out.enc   = encoding || 'utf-8';
        out.lang  = lang     || 'en';
      } else if (recordType === 'url') {
        out.value = new TextDecoder().decode(data);
      } else if (recordType === 'mime' || mediaType) {
        out.type  = mediaType || recordType;
        out.value = (mediaType || '').startsWith('text/')
          ? new TextDecoder().decode(data)
          : this._toHex(data);
        out.isHex = !(mediaType || '').startsWith('text/');
      } else if (recordType === 'smart-poster') {
        out.value = '(Smart Poster — see nested records)';
      } else {
        out.value = this._toHex(data);
        out.isHex = true;
      }
    } catch {
      out.value = this._toHex(data);
  ...
  private _toHex(dataView: DataView | ArrayBuffer): string {
    const bytes = new Uint8Array((dataView as any).buffer ?? dataView);
    ...
    return html`
      ...
      <main>
        ...
        <div class="content">
          ...
          ${this.showCompat ? html`
            <div class="compat-box">${this.compatMsg}</div>
          ` : ''}

          <!-- Scanner card -->
          <div class="scanner-card">
            <div class="pulse-wrap ${isScanning ? 'scanning' : ''}">
              <div class="ring"></div>
              <div class="ring"></div>
              <div class="ring"></div>
            ...
            <div class="result-panel">
              ...
              ${this.tags.map(tag => html`
                <div class="record-item">
                  ${tag.serialNumber ? html`
                    <div class="record-meta">
                      <span class="badge badge-type">UID</span>
                      <span class="record-data" style="flex:1;">${tag.serialNumber}</span>
                      <span class="badge badge-time">${tag.time}</span>
                    </div>
                  ` : ''}

                  ${tag.records.map((rec, i) => html`
                    <div class="record-meta">
                      <span class="badge badge-type">${rec.type}</span>
                      ${rec.enc  ? html`<span class="badge badge-enc">${rec.enc}</span>`         : ''}
                      ${rec.lang ? html`<span class="badge badge-lang">lang:${rec.lang}</span>` : ''}
                      ${!tag.serialNumber && i === 0
                        ? html`<span class="badge badge-time">${tag.time}</span>` : ''}
                    </div>
                    <div class="record-data ${rec.isHex ? 'hex' : ''}">
                      ${rec.value ?? '(empty)'}
                    </div>
                  `)}
                </div>
