const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["code/app-about-JyWZEWgq.js","code/property-QzOEr6Ld.js","code/class-map-wFiN0yRb.js","code/app-rfid-BrxATc2d.js","code/log-store-BSOZGDb1.js","code/app-ble-Bbsp73O8.js","code/app-log-BgGd_IAo.js"])))=>i.map(i=>d[i]);
import{n as L,x as v,i as R,r as P,t as E}from"./property-QzOEr6Ld.js";function O(n){return L({...n,state:!0,attribute:!1})}const k="modulepreload",$=function(n){return"/pwa-airsampler/"+n},b={},f=function(e,a,i){let r=Promise.resolve();if(a&&a.length>0){let w=function(c){return Promise.all(c.map(h=>Promise.resolve(h).then(u=>({status:"fulfilled",value:u}),u=>({status:"rejected",reason:u}))))};document.getElementsByTagName("link");const t=document.querySelector("meta[property=csp-nonce]"),s=t?.nonce||t?.getAttribute("nonce");r=w(a.map(c=>{if(c=$(c),c in b)return;b[c]=!0;const h=c.endsWith(".css"),u=h?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${u}`))return;const p=document.createElement("link");if(p.rel=h?"stylesheet":k,h||(p.as="script"),p.crossOrigin="",p.href=c,s&&p.setAttribute("nonce",s),document.head.appendChild(p),h)return new Promise((y,_)=>{p.addEventListener("load",y),p.addEventListener("error",()=>_(new Error(`Unable to preload CSS for ${c}`)))})}))}function o(t){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=t,window.dispatchEvent(s),!s.defaultPrevented)throw t}return r.then(t=>{for(const s of t||[])s.status==="rejected"&&o(s.reason);return e().catch(o)})},x=Symbol.for("app-tools::log::1.x");globalThis[x]={setDebug:z,debug:"window"in globalThis?new URL(window.location.href).searchParams.has("app-tools-debug"):!1};function z(n){globalThis[x].debug=!!n}function A(n,e){globalThis[x].debug&&(console.groupCollapsed(`[app-tools] ${n}`),e&&console.log(e),console.groupEnd())}function U(n){return(e,a)=>{A(`${n}: ${e}`,a)}}const d=U("router");class S extends Event{constructor(e){super("route-changed"),this.context=e}}class C extends EventTarget{context={params:{},query:{},title:"",url:new URL(window.location.href)};constructor(e){super(),this.config=e,this.routes=e.routes.map(a=>({...a,urlPattern:new URLPattern({pathname:a.path,baseURL:window.location.href,search:"*",hash:"*"})})),d("Initialized routes",this.routes),queueMicrotask(()=>{this.navigate(new URL(window.location.href),{replace:!0})}),window.addEventListener("popstate",this._onPopState),window.addEventListener("click",this._onAnchorClick)}uninstall(){window.removeEventListener("popstate",this._onPopState),window.removeEventListener("click",this._onAnchorClick)}get url(){return new URL(window.location.href)}get fallback(){return new URL(this.config?.fallback||this.baseUrl.href.substring(window.location.origin.length),this.baseUrl)}get baseUrl(){return new URL("./",document.baseURI)}render(){return d(`Rendering route ${this.context.url.pathname}${this.context.url.search}${this.context.url.hash}`,{context:this.context,route:this.route}),this.route?.render?.(this.context)}_matchRoute(e){for(const a of this.routes){const i=a.urlPattern.exec(e);if(i){const{title:r}=a,o=Object.fromEntries(new URLSearchParams(e.search)),t=i?.pathname?.groups??{};return this.context={url:e,title:typeof r=="function"?r({params:t,query:o,url:e}):r,params:t,query:o},a}}return d(`No route matched for ${e.pathname}${e.search}${e.hash}`,e),null}_notifyUrlChanged(){this.dispatchEvent(new S(this.context))}_onPopState=()=>{this.navigate(new URL(window.location.href),{backNav:!0})};_onAnchorClick=e=>{if(e.defaultPrevented||e.button!==0||e.metaKey||e.ctrlKey||e.shiftKey)return;const a=e.composedPath().find(o=>o.tagName==="A");if(!a||!a.href)return;const i=new URL(a.href);if(this.url.href===i.href||i.host!==window.location.host||a.hasAttribute("download")||a.href.includes("mailto:"))return;const r=a.getAttribute("target");r&&r!==""&&r!=="_self"||(e.preventDefault(),this.navigate(i))};_collectPlugins(e){return[...this.config?.plugins??[],...e?.plugins??[]]}async navigate(e,a={}){typeof e=="string"&&(e=new URL(e,this.baseUrl));let i=this._matchRoute(e)||this._matchRoute(this.fallback);d(`Navigating to ${e.pathname}${e.search}${e.hash}`,{context:this.context,route:this.route});let r=this._collectPlugins(i);for(const o of r)try{const t=await o?.shouldNavigate?.(this.context);t&&(await t.condition()||(e=new URL(t.redirect,this.baseUrl),i=this._matchRoute(e)||this._matchRoute(this.fallback),r=this._collectPlugins(i),d("Redirecting",{context:this.context,route:this.route})))}catch(t){throw d(`Plugin "${o.name}" error on shouldNavigate hook`,t),t}if(this.route=i,!this.route)throw new Error(`[ROUTER] No route or fallback matched for url ${e}`);for(const o of r)try{await o?.beforeNavigation?.(this.context)}catch(t){throw d(`Plugin "${o.name}" error on beforeNavigation hook`,t),t}a?.replace?window.history.replaceState(null,"",`${e.pathname}${e.search}${e.hash}`):a.backNav||window.history.pushState(null,"",`${e.pathname}${e.search}${e.hash}`),document.title=this.context.title,this._notifyUrlChanged();for(const o of r)try{await o?.afterNavigation?.(this.context)}catch(t){throw d(`Plugin "${o.name}" error on afterNavigation hook`,t),t}}}function g(n){return{name:"lazy",beforeNavigation:()=>{n()}}}globalThis.URLPattern||await f(()=>import("./index-DkuV2QLQ.js"),[]);const N="/pwa-airsampler/",I=new C({routes:[{path:l(),title:"Home",render:()=>v`<app-home></app-home>`},{path:l("about"),title:"About",plugins:[g(()=>f(()=>import("./app-about-JyWZEWgq.js"),__vite__mapDeps([0,1,2])))],render:()=>v`<app-about></app-about>`},{path:l("rfid"),title:"RFID Scanner",plugins:[g(()=>f(()=>import("./app-rfid-BrxATc2d.js"),__vite__mapDeps([3,1,4])))],render:()=>v`<app-rfid></app-rfid>`},{path:l("ble"),title:"BLE Control",plugins:[g(()=>f(()=>import("./app-ble-Bbsp73O8.js"),__vite__mapDeps([5,1,4])))],render:()=>v`<app-ble></app-ble>`},{path:l("log"),title:"Log Viewer",plugins:[g(()=>f(()=>import("./app-log-BgGd_IAo.js"),__vite__mapDeps([6,1,4])))],render:()=>v`<app-log></app-log>`}]});function l(n){var e=N;return n&&(e=e+n),e}var T=Object.getOwnPropertyDescriptor,V=(n,e,a,i)=>{for(var r=i>1?void 0:i?T(e,a):e,o=n.length-1,t;o>=0;o--)(t=n[o])&&(r=t(r)||r);return r};let m=class extends P{render(){return v`
  ...
  `;m=V([E("app-home")],m);export{O as a,I as b,l as r};
//# sourceMappingURL=app-home-37ANSCbe.js.map
